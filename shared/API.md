...
| `useJiraAuth(baseUrl)` | Per-user Jira OAuth 2.0 (3LO) flow. Exports `{ connected, connecting, siteName, siteUrl, connectJira, disconnectJira, checkConnectionStatus }`. Handles popup auth, token storage in session cookies. `baseUrl` is module API prefix (e.g., `/api/modules/customer-insights`). |
...
| `jiraOAuth` | `{ registerJiraOAuthRoutes }` — Per-user Jira OAuth 2.0 (3LO) infrastructure. `registerJiraOAuthRoutes(router, { clientId, clientSecret, callbackUrl?, scopes? })` registers OAuth routes (`/auth/jira`, `/auth/jira/callback`, `/auth/jira/status`, `/auth/jira/disconnect`) and attaches `router.getJiraClient(req)` helper. Returns the router. `getJiraClient(req)` returns `Promise<{ jiraRequest, cloudId }>`, handles token refresh, throws if not authenticated. Tokens stored in `req.session` (requires `express-session`). Scopes default to `['write:jira-work', 'read:jira-user', 'offline_access']`. |
