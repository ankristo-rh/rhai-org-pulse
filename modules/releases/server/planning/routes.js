...
var { queryFeaturesFromJira } = require("./feature-query")
var { fetchAllJqlResults } = require("../../../../shared/server/jira")
...
module.exports = function registerPlanningRoutes(router, context) {
  ...
  router.get('/feature-readiness', requireAuth, requireScope('releases:read'), async function(req, res) {
    try {
      ...
      if (jiraClient) {
        try {
          jiraFeatures = await queryFeaturesFromJira(jiraClient.jiraRequest, fetchAllJqlResults, readFromStorage)
        } catch (jiraErr) {
          console.warn('[releases/planning] Jira query failed, falling back to cache-only:', jiraErr.message)
        }
      }
